...
class IronWorkorderEditScreen extends StatefulWidget {
  final String workorderId;
  const IronWorkorderEditScreen({super.key, required this.workorderId});

  @override
  State<IronWorkorderEditScreen> createState() =>
      _IronWorkorderEditScreenState();
}

class _IronWorkorderEditScreenState extends State<IronWorkorderEditScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      provider.loadWorkOrderById(widget.workorderId).then((_) {
        if (provider.workOrderDetail != null) {
          // Set initial form values
          _formKey.currentState?.patchValue({
            'client_id': provider.clients.firstWhere(
              (client) => client.id == provider.selectedClient,
              orElse: () => Client(),
            ),
            'project_id': provider.projects.firstWhere(
              (project) => project.id == provider.selectedProject,
              orElse: () => Project(),
            ),
            'work_order_number': provider.workOrderNumber,
            'work_order_date': provider.workOrderDate ?? DateTime.now(),
            'uploaded_files': provider.uploadedFiles,
          });

          // Patch product fields
          provider.products.asMap().forEach((index, product) {
            _formKey.currentState?.fields['shape_code_$index']?.didChange(
              product['shapeCode'],
            );
            _formKey.currentState?.fields['member_detail_$index']?.didChange(
              product['memberDetail'],
            );
            _formKey.currentState?.fields['bar_mark_$index']?.didChange(
              product['barMark'],
            );
            _formKey.currentState?.fields['delivery_date_$index']?.didChange(
              product['deliveryDate'] ?? DateTime.now(),
            );
            _formKey.currentState?.fields['member_quantity_$index']?.didChange(
              product['memberQuantity']?.toString(),
            );
            _formKey.currentState?.fields['uom_$index']?.didChange(
              product['uom'],
            );
            _formKey.currentState?.fields['po_quantity_$index']?.didChange(
              product['quantity']?.toString(),
            );
            _formKey.currentState?.fields['diameter_$index']?.didChange(
              product['diameter'],
            );
            _formKey.currentState?.fields['weight_$index']?.didChange(
              product['weight']?.toString(),
            );

            // Patch dimension fields
            product['dimensions'].forEach((key, value) {
              _formKey.currentState?.fields[key]?.didChange(value?.toString());
            });
          });
        }
      });
        ...
        builder: (context, provider, child) {
          return Container(
            ...
            child: Scaffold(
              ...
              appBar: AppBars(
                title: TitleText(title: 'Edit Work Order'),
